...
import logging
class AfvalwijzerCustom(object):

    def __init__(self, zipcode, housenumber):
        self.housenumber = housenumber
        logging.warning('from afvalwijzer')
    ...
    def __get_data(self):
        ...
        pickupdates = [str(x.text) for x in soup]
    ...
    def getData(self):
        glist = self.pickupdates[2].split('\n')
        print(glist)
        index = 0
        for line in glist:
            if "overzicht per maand" in line.lower():
                print(index)
                glist = glist[index:]
                break
            index += 1
        return [x for x in glist if x != '']